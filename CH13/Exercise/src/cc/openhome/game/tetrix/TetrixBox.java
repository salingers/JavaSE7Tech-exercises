...
public class TetrixBox extends JComponent {
    private int boxWidthInBlk, boxHeightInBlk;
    ...
    public TetrixBox(int boxWidthInBlk, int boxHeightInBlk, 
                          int blockWidth, int blockHeight) {
        super();
        this.boxWidthInBlk = boxWidthInBlk;
        this.boxHeightInBlk = boxHeightInBlk;
    ...
    private void drawBlock(Graphics g, int x, int y) {
        g.drawImage(currentImage, x*blockWidth, y*blockHeight, blockWidth, blockHeight, this);
        if(isBlockBorder) {
    ...
    public void paint(Graphics g) {
        ...
        for(int i = 0; i < 4; i++) {
            drawBlock(g, currentPiece.getBlockXCoord(i) + 1, currentPiece.getBlockYCoord(i)+1);
        }
    }
}
