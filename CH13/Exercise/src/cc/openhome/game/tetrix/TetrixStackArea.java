package cc.openhome.game.tetrix;
...
public class TetrixStackArea extends JComponent {
    ...
    public TetrixStackArea(TetrixBox tetrixBox, TetrixGround tetrixGround) {
        ...
        setSize(tetrixGround.getGroundWidthInBlk()*tetrixBox.getBlockWidth(), 
                tetrixGround.getGroundHeightInBlk()*tetrixBox.getBlockHeight());
    }
    ...
    private void drawBlock(Graphics g, Image image, int x, int y) {
        g.drawImage(image, x*tetrixBox.getBlockWidth(), y*tetrixBox.getBlockHeight(), 
                tetrixBox.getBlockWidth(), tetrixBox.getBlockHeight(), this);
        
        if(isBlockBorder) {
            g.setColor(Color.lightGray);
            g.drawRect(x*tetrixBox.getBlockWidth(), y*tetrixBox.getBlockHeight(), 
    ...
    public void paint(Graphics g) {
        ...
        // draw current piece
        for(int i = 0; i < 4; i++) {
            drawBlock(g, tetrixBox.getTetrixImages()[tetrixGround.getTetrixPiece().getType()-1],
        ...
        for(int i = 0; i < tetrixGround.getGroundWidthInBlk(); i++) {
            for(int j = 0; j < tetrixGround.getGroundHeightInBlk(); j++) {
                ...
                drawBlock(g, tetrixBox.getTetrixImages()[tetrixGround.getGroundInfo()[i][j] - 1], i, j);
            }
        }

        if(tetrixGround.isGameover()) {
            g.setFont(new Font("Arial Black", Font.ITALIC | Font.BOLD, tetrixBox.getBlockWidth()));
